...
# ===================== INTERFAZ STREAMLIT =====================
st.title("Cálculo de posición por Rectas de Altura con Desplazamiento")
...
distancia = st.number_input("Distancia del desplazamiento (millas)", step=0.1, value=0.0)
...
if st.button("Calcular"):
    ...
    # Ajustes por diferencia de altura
    if dh1t < 0:
        azimut1 += 180
    ...
    # Vectores
    ax.plot([0, dx1 - dx0], [0, dy1 - dy0], 'y', linewidth=2, label="Vector 1")
    ax.plot([dx1 - dx0, dx1], [dy1 - dy0, dy1], 'b', linewidth=2, label="Desplazamiento")
    ...
    ax.plot([dx1 - perp_dx1, dx1 + perp_dx1], [dy1 - perp_dy1, dy1 + perp_dy1], 'r--', linewidth=2)
    ax.plot([dx2 - perp_dx2, dx2 + perp_dx2], [dy2 - perp_dy2, dy2 + perp_dy2], 'r--', linewidth=2)
